...
extension NeedleTailCrypto {
    ...
    public func saveKeychainPrivateKey(configuration: KeychainConfiguration, with alogrythm: CryptoAlogrythm) async throws {
        ...
        switch alogrythm {
        case .curve25519:
            key = try generateCurve25519PrivateKey().encodedKey
        case .p256:
            key = try generateP256PrivateKey().encodedKey
        case .p384:
            key = try generateP384PrivateKey().encodedKey
        case .p521:
            key = try generateP521PrivateKey().encodedKey
        case .secureEnclave:
